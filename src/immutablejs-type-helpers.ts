...
// ...
import { fromJS, Iterable, Map, OrderedMap, Set } from 'immutable'
...
const get = curry(<T>(imObj: Map<string, T>, key: string) => {
  return imObj.get(key)
})

const convertToMap = <T extends { [key: string]: any }>(items: Set<T>, key = 'id') => {
  return Map().withMutations(mut => {
    items.forEach((item?: T) => {
      if (!item) {
        return
      }
      mut.set(item[key], fromJS(item))
    })
  })
}
 ...
 */
export const isArray = (maybeArray: any) =>
  !!(
    maybeArray &&
    typeof maybeArray === 'object' &&
    typeof maybeArray !== 'string' &&
    maybeArray.constructor === Array
  )
 ...
 */
export const getValid = (maybeIterable: any, Default: any) =>
  (isIterable(maybeIterable) && maybeIterable) || (isIterable(Default) && Default)
...
export const getValidPath = (maybeIterable: any, keyPath: any, Default: any) => {
  return isIterable(maybeIterable) && keyPath
    ? (isArray(keyPath) && maybeIterable.getIn(keyPath, Default)) ||
        maybeIterable.get(keyPath, Default)
    : getValid(maybeIterable, null) || Default || false
}

export const mapItems = <T extends { [key: string]: any }>(
  items: Set<T>,
  key = 'id',
  keepOrder = false,
  valueKey: string = ''
